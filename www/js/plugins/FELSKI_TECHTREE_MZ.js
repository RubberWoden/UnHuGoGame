...
var parameters = PluginManager.parameters('FELSKI_TECHTREE_MZ');
...
Felski.Techtree.windowUpdates = 1;
...
//************************************************************************************************
    ...
    Felski.Techtree.Scene_Boot_loadSystemImages = Scene_Boot.prototype.loadSystemImages;
    Scene_Boot.prototype.loadSystemImages = function() {
        Felski.Techtree.Scene_Boot_loadSystemImages.call(this);
        ImageManager._techtreeImages = {};
        ImageManager._techtreeImages.Tech_Hidden = ImageManager.reserveTechtree('Tech_Hidden');
        ImageManager._techtreeImages.Tech_Inactive = ImageManager.reserveTechtree('Tech_Inactive');
        ImageManager._techtreeImages.Tech_Unlockable = ImageManager.reserveTechtree('Tech_Unlockable');
        ImageManager._techtreeImages.Tech_Active = ImageManager.reserveTechtree('Tech_Active');
        ImageManager._techtreeImages.Arrows = ImageManager.reserveTechtree('Arrows');
        ImageManager._techtreeImages.Arrows_Active = ImageManager.reserveTechtree('Arrows_Active');
    };
	...
	ImageManager.loadTechtree = function(filename, hue) {
	    return ImageManager._techtreeImages[filename];
	};
...
//************************************************************************************************
    ...
    Felski.Techtree.Window_MenuStatus_processOk = Window_MenuStatus.prototype.processOk;
    Window_MenuStatus.prototype.processOk = function() {
	    Felski.Techtree.Window_MenuStatus_processOk.call(this);
	};
...
//************************************************************************************************
    ...
    Scene_Techtree.prototype.preloadImages = function(){
    	...
    	for (var i = 0; i < actors.length; i++) {
    		var actor = actors[i];
    		if(!!!actor) continue;
	    	console.log("Preloading image for actor: "+actor.name());
	    	if(actor._techtrees){
				for (var j = 0; j < actor._techtrees.length; j++) {
	    			if(actor._techtrees[j].bgimg){ //preload tree images
	    				ImageManager.reserveTechtreeBGIMG(actor._techtrees[j].bgimg);
	    				console.log("Preloading tree image: "+actor._techtrees[j].bgimg);
	    			}
	    			for (var h = 0; h < actor._techtrees[j].nodes.length; h++) { // preload node images
	    				if(actor._techtrees[j].nodes[h].technode.bgimg){
	    					ImageManager.reserveTechtreeBGIMG(actor._techtrees[j].nodes[h].technode.bgimg);
	    					console.log("Preloading node image: "+actor._techtrees[j].nodes[h].technode.bgimg);
	    				}
	    			}
	    		}
			}
...
//************************************************************************************************
    ...
    Window_Techtree.prototype.initialize = function(x, y, h2, actor, startTree) {
        ...
        this._changed = Felski.Techtree.windowUpdates;
		this._buttonSprites = [];
    ...
    Window_Techtree.prototype.changebgimg = function() {
        ...
        if(this._activeTree){
        	if(this._activeTree.bgimg){
				...
				this.contentsBack.bitmap = new Bitmap(newWidth, newHeight);
				this.contentsBack.bitmap.blt(bitmap, 0, 0, bitmap.width, bitmap.height, 0, 0, newWidth, newHeight);
        	}else{
...
//************************************************************************************************
    ...
    Window_Techtree.prototype.laneHeight = function() {
        if(Felski.Techtree.horizontal){
            return (this.windowHeight() - $gameSystem.windowPadding()) / Felski.Techtree.Lanes;
        }else{
    ...
    Window_Techtree.prototype.laneWidth = function() {
        if(Felski.Techtree.horizontal){
            ...
            return (this.windowWidth() - $gameSystem.windowPadding()) / Felski.Techtree.Lanes;
        }
    };
...
//************************************************************************************************
    ...
    Window_Techtree.prototype.drawActorInfo = function() {
        ...
        this.changeTextColor(ColorManager.textColor(Felski.Techtree.TTFontColor));
    ...
    Window_Techtree.prototype.drawHeader = function() {
        this.clearSprites(); 
        var x = 200 + $gameSystem.windowPadding();
        var y = 0;
        var y2 = $gameSystem.windowPadding();
        var ip = ImageManager.iconWidth + $gameSystem.windowPadding();
    ...
    Window_Techtree.prototype.drawNode = function(nodeData) {
        ...
        this.changeTextColor(ColorManager.textColor(Felski.Techtree.TTNHFontColor));
        ...
        if(this.isNodeVisible(nodeData)){
            ...
            if(!Felski.Techtree.NodesCostHidden) {
	            if(nodeData.active && Felski.Techtree.NodesLearnedIconShown){
	            	...
	            	mid = mid - ImageManager.iconWidth/2;
            ...
            this.drawBoxFrameBackgroundBitmap(x, y, rect.width, rect.height, bitmap, Felski.Techtree.NodeOpacity);
            this.drawBoxFrameBitmap(x, y, rect.width, rect.height, bitmap);
            x = rect.x + (rect.width - ImageManager.iconWidth)/2;
            y = rect.y + (rect.height - ImageManager.iconHeight)/2;
    ...
    Window_Techtree.prototype.drawCosts = function(rect, costs) {
        ...
        this.changeTextColor(ColorManager.textColor(Felski.Techtree.TTNCFontColor));
        this.setFontSize(Felski.Techtree.TTNCFontSize);
        var hp = $gameSystem.windowPadding()/2;
        var ip = ImageManager.iconWidth + hp;
...
//************************************************************************************************
    ...
    Window_Techtree.prototype.getNodeRect = function(technode) {
        ...
        if(Felski.Techtree.horizontal){
            ...
            y = y + (Felski.Techtree.NodeHeight + $gameSystem.windowPadding()) * lane;        
        }else{
            x = x + (Felski.Techtree.NodeWidth + $gameSystem.windowPadding()) * lane;
...
//************************************************************************************************
//
// Window_Techtree // MZ Fixes
//
//************************************************************************************************

    Window_Techtree.prototype.drawActorFace = function(actor, x, y, width, height) {
	    this.drawFace(actor.faceName(), actor.faceIndex(), x, y, width, height);
	};

    Window_Techtree.prototype.canvasToLocalX = function(x) {
        var node = this;
        while (node) {
            x -= node.x;
            node = node.parent;
    ...
    Window_Techtree.prototype.canvasToLocalY = function(y) {
        var node = this;
        while (node) {
            y -= node.y;
            node = node.parent;
...
//************************************************************************************************
	...
	Window_Techtree_Select.prototype.drawItem = function(index) {
		...
		if(this.isCommandVisible(index)) {
		    var rect = this.itemLineRect(index - this._firstVisibleCol);
			...
			this.changeTextColor(ColorManager.textColor(Felski.Techtree.TTHeaderFontColor));
			...
			if(Felski.Techtree.TTHeaderTIcons) this.drawIcon(tree.icon, rect.x + rect.width-ImageManager.iconWidth, rect.y+2);
		    this.drawText(this.commandName(index), rect.x, rect.y, rect.width, align);

		    if(this.index() === index){
		    	var desc = tree.tech_description;
		    	this.setFont(Felski.Techtree.TTHeaderDescFont);
				this.changeTextColor(ColorManager.textColor(Felski.Techtree.TTHeaderDescFontColor));
				this.setFontSize(Felski.Techtree.TTHeaderDescFontSize);
		    	this.drawTextAutoWrap(desc, 0, rect.height, this.windowWidth() - $gameSystem.windowPadding()*2);
		    }
		    this.resetTextColor();
	    }
	};
...
//************************************************************************************************
	...
	Window_Techtree_Info.prototype.windowWidth = function() {
	    return ImageManager.faceWidth + $gameSystem.windowPadding();
	};

	Window_Techtree_Info.prototype.windowHeight = function() {
	    return ImageManager.faceHeight + $gameSystem.windowPadding();
	};
...
//************************************************************************************************
//
// Window_Techtree_Info // MZ Fixes
//
//************************************************************************************************

    Window_Techtree_Info.prototype.drawActorFace = function(actor, x, y, width, height) {
	    this.drawFace(actor.faceName(), actor.faceIndex(), x, y, width, height);
	};
...
//************************************************************************************************
    ...
    Window_Techtree_Confirm.prototype.initialize = function(actor, onlyDisplay) {
        ...
        this._firstDraw = Felski.Techtree.windowUpdates;
        var width = this.windowWidth();
        var height = this.windowHeight();
        var rect = new Rectangle(x, y, width, height)
	    Window_Base.prototype.initialize.call(this, rect);
    ...
    Window_Techtree_Confirm.prototype.changebgimg = function() {
        ...
        if(this._activeTree){
        	if(this._node.technode.bgimg){
				...
				var newWidth = Math.floor(this.windowWidth() - $gameSystem.windowPadding());
				var newHeight = Math.floor(this.windowHeight() - $gameSystem.windowPadding());
...
//************************************************************************************************
    ...
    Window_Techtree_Confirm.prototype.drawInfo = function() {
        ...
        this.changeTextColor(ColorManager.textColor(Felski.Techtree.TTNTFontColor));
    ...
    Window_Techtree_Confirm.prototype.drawRequirement = function() {
        ...
        if(technode.levelRequirement){
	        ...
	        this.changeTextColor(ColorManager.textColor(Felski.Techtree.TTLevelRequirementFontColor));
    ...
    Window_Techtree_Confirm.prototype.drawActionTexts = function() {
        var x = 0;
        var y = this.windowHeight() - this.techTreeLineHeight() - $gameSystem.windowPadding()*2;
        var w = (this.windowWidth()-$gameSystem.windowPadding()*2)/2;
        ...
        if(this.canBeUnlocked() && this.canPayCost()){
            ...
            this.changeTextColor(ColorManager.textColor(Felski.Techtree.TTNHFontColorGreyedOut));
        }
        if(this.isNodeActive()){
    ...
    Window_Techtree_Confirm.prototype.drawNodeInfo = function(withRequirement) {
        ...
        if(Felski.Techtree.TTNShowTechDesc){
        	line = line + this.drawTextAutoWrap(technode.tech_description, x, lh*line, w-$gameSystem.windowPadding());
        }
        console.log(line)
        // onActivate
        ...
        for (var i = 0; i < onActivate.skills.length; i++) {
            ...
            if(Felski.Techtree.TTNShowSkillDesc) line = line + this.drawTextAutoWrap(skill.description, x+16, lh*line, (w-ip-16-$gameSystem.windowPadding()));
        }
        ...
        if(onActivate.stats){
        	...
        	for (var i = 0; i < statsArr.length; i++) {
        		...
        		var w2;
        		if(nextLine){
        			w2 = (w - $gameSystem.windowPadding()) / (statsArr.length - Math.ceil(statsArr.length/2));
        		}else{
        			w2 = (w - $gameSystem.windowPadding()) / Math.ceil(statsArr.length/2);
        		}
    ...
    Window_Techtree_Confirm.prototype.drawCosts = function(costs) {
        ...
        var y = this.windowHeight() - this.lineHeight()*2 - $gameSystem.windowPadding()*2;
        for (var i = 0; i < costs.length; i++) {
            tw = this.textWidth(costs[i].amount);
            this.drawText(costs[i].amount, x, y, tw, 'left');
            x = x+tw+hp;
...
//************************************************************************************************
//
// Window_Techtree_Confirm - MZ Fixes
//
//************************************************************************************************

    Window_Techtree_Confirm.prototype.canvasToLocalX = function(x) {
        var node = this;
        while (node) {
            x -= node.x;
            node = node.parent;
    ...
    Window_Techtree_Confirm.prototype.canvasToLocalY = function(y) {
        var node = this;
        while (node) {
            y -= node.y;
            node = node.parent;
...
//************************************************************************************************
	...
	Window_Techtree_Currency.prototype.windowWidth = function() {
	    return (ImageManager.faceWidth + $gameSystem.windowPadding())*1;
	};
	...
	Window_Techtree_Currency.prototype.refresh = function() {
	    this.contents.clear();
	    var lh = this.techTreeLineHeight();
        var sp = $gameSystem.windowPadding();
	    var w = ImageManager.faceWidth;
	    var x = 0;
	    var y = 0;
        var x2 = w - ImageManager.iconWidth - sp;
        var y2 = w/2 - lh/2;
        var w2 = x2;

        this.setFont(Felski.Techtree.TTHeaderCurFont);
        this.changeTextColor(ColorManager.textColor(Felski.Techtree.TTHeaderCurFontColor));
...
//************************************************************************************************
	...
	Sprite_Techtree_Animation.prototype = Object.create(Spriteset_Menu.prototype);
	...
	Sprite_Techtree_Animation.prototype.initialize = function(animation) {
		Spriteset_Menu.prototype.initialize.call(this);
		this.z = 9;
		this._realX = this.x;
		this._realY = this.y;
	...
	Sprite_Techtree_Animation.prototype.update = function() {
		Spriteset_Menu.prototype.update.call(this);
		if (!this._hasStarted && this.parent) {
			var target = {};
			target.x = this.x;
			target.y = this.y;
			this.createAnimationSprite(target, this._animation, false, 0);
...
//************************************************************************************************
	...
	Spriteset_Menu.prototype.destroy = function(options) {
	    Spriteset_Base.prototype.destroy.call(this, options);
	};

	Spriteset_Menu.prototype.findTargetSprite = function(target) {
	    return target;
	};
